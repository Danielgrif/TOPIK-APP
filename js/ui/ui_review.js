...
import { ensureSessionStarted } from './ui.js';
import { updateSRSBadge } from '../core/stats.js';
import { scheduleSaveState } from '../core/db.js';

/**
 * Opens the SRS Review Mode modal.
 */
...
export function buildReviewModal(queue) {
    ...
    function renderOne() {
        ...
        const handleReview = (grade) => {
            ...
            // FIX: Всегда помечаем слово для синхронизации
            state.dirtyWordIds.add(w.id);

            if (grade >= 3) {
                stats.remembered++;
                // Если вспомнил - убираем из ошибок
            ...
            // FIX: Явный вызов сохранения, так как Scheduler не может вызвать его через window
            scheduleSaveState();

            const d = Math.round(res.interval * 10) / 10; // Round for display
