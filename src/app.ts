...
import { fetchVocabulary, loadFromSupabase, immediateSaveState } from './core/db.ts';
import { 
    toggleSessionTimer,
    sortByWeakWords, shuffleWords, toggleViewMode,
    showError, saveAndRender
} from './ui/ui.ts'; // Ensure this file is in ui/ folder
import { showUpdateNotification, setupGestures, setupScrollBehavior, saveSearchHistory, showSearchHistory, hideSearchHistory, showInstallBanner, dismissInstallBanner } from './ui/ui_interactions.ts';
import { 
...
import { render, renderSkeletons, resetSearchHandler, setupGridEffects } from './ui/ui_card.ts';
import { openModal, closeModal, openConfirm, closeConfirm } from './ui/ui_modal.ts';
import { toggleHanjaMode, toggleVoice, updateVoiceUI, toggleDarkMode, toggleAutoUpdate, applyTheme, toggleFocusMode, applyFocusMode, toggleBackgroundMusic, setBackgroundMusicVolume, applyBackgroundMusic, setAccentColor, setAudioSpeed } from './ui/ui_settings.ts';
...
import { renderTopicMastery, updateXPUI, updateStats, updateSRSBadge, renderDetailedStats } from './core/stats.ts';
import { startDailyChallenge, updateDailyChallengeUI, checkSuperChallengeNotification, quitQuiz } from './ui/quiz.ts';
    ...
    document.body.addEventListener('click', (e) => {
        ...
        if (actionTrigger) {
            ...
            switch (action) {
                ...
                case 'set-type-filter': if (value) setTypeFilter(value, actionTrigger as HTMLElement); break;
                case 'set-star-filter': if (value) setStarFilter(value, actionTrigger as HTMLElement); break;
                case 'sort-weak': sortByWeakWords(); break;
                case 'shuffle': shuffleWords(); break;
                case 'open-review': import('./ui/ui_review.ts').then(m => m.openReviewMode()); break;
                case 'set-accent': 
                ...
                case 'share-stats': 
                    ...
                    import('./ui/ui_share.ts').then(m => m.shareStats(color ?? undefined)); 
                    break;
                case 'install-app': 
                ...
                case 'submit-word-request': import('./ui/ui_custom_words.ts').then(m => m.submitWordRequest()); break;
                ...
                case 'toggle-hanja': toggleHanjaMode(actionTrigger.querySelector('input') || actionTrigger as HTMLInputElement); break;
                case 'toggle-voice': toggleVoice(); break;
                case 'toggle-music': toggleBackgroundMusic(actionTrigger.querySelector('input') || actionTrigger as HTMLInputElement); break;
                case 'toggle-auto-update': 
                    const el = actionTrigger.querySelector('input') || actionTrigger as HTMLInputElement;
                    toggleAutoUpdate(el);
...
Object.assign(window, {
    ...
    saveAndRender,
    importProgress: (event: Event) => import('./ui/ui_data.js').then(m => m.importProgress(event)),
    ...
    setAudioSpeed: (val: string | number) => import('./ui/ui_settings.ts').then(m => m.setAudioSpeed(val)),
    signInWithGoogle,
    speak,
    openLoginModal,
    openReviewMode: () => import('./ui/ui_review.ts').then(m => m.openReviewMode()),
    openShopModal: () => import('./ui/ui_shop.ts').then(m => m.openShopModal()),
    startDailyChallenge,
    quitQuiz,
    checkPronunciation: (word: string, btn: HTMLElement) => import('./core/speech.js').then(m => m.checkPronunciation(word, btn)),
    resetSearchHandler,
