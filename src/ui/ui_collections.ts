...
export let userLists: UserList[] = [];
export let listItems: Record<string, Set<number>> = {}; // listId -> Set<wordId>
export let currentCollectionFilter: string | null = null;
...
export async function loadCollections() {
  ...
  const { data: items, error: itemsError } = await client.from('list_items').select('*');
  ...
  items?.forEach((item: any) => {
    if (!listItems[item.list_id]) listItems[item.list_id] = new Set();
    listItems[item.list_id].add(item.word_id);
  });
...
export async function createList() {
  ...
  const iconInput = document.getElementById('new-list-icon') as HTMLInputElement;
  const publicCheck = document.getElementById('new-list-public') as HTMLInputElement;
  ...
  const btn = document.querySelector('[data-action="create-list"]') as HTMLButtonElement;
  let originalContent = "";
  if (btn) {
...
export function deleteList(listId: string, btn?: HTMLElement) {
  openConfirm("Удалить этот список?", async () => {
    ...
    const listBackup = userLists[listIndex];
    const itemsBackup = listItems[listId];
    const wasActiveFilter = currentCollectionFilter === listId;
    ...
    showUndoToast(
        ...
        () => {
            // Undo
            userLists.splice(listIndex, 0, listBackup);
            listItems[listId] = itemsBackup;
            if (wasActiveFilter) {
                currentCollectionFilter = listId;
                render();
            }
            updateCollectionUI();
        },
...
export function openEditListModal(listId: string, currentTitle: string, currentIcon: string) {
  const idInput = document.getElementById('edit-list-id') as HTMLInputElement;
  const titleInput = document.getElementById('edit-list-title') as HTMLInputElement;
  const iconInput = document.getElementById('edit-list-icon') as HTMLInputElement;
...
export async function saveListChanges() {
  ...
  if (!newTitle) { showToast("Название не может быть пустым"); return; }
  
  const { error } = await client.from('user_lists').update({ title: newTitle, icon: newIcon }).eq('id', listId);
  if (error) { showToast("Ошибка: " + error.message); return; }
  
  const list = userLists.find(l => l.id === listId);
  if (list) { list.title = newTitle; list.icon = newIcon; }
  
  updateCollectionUI();
  if (currentCollectionFilter === listId) {
       const filterBtn = document.getElementById('collection-filter-btn');
       if (filterBtn) filterBtn.innerHTML = `<span>${newIcon} ${newTitle}</span> <span>✕</span>`;
  }
...
export function updateCollectionUI() {
  ...
  if (container) {
    ...
    client.auth.getSession().then(({ data: { session } }: any) => {
        ...
        if (lists.length === 0) {
            html += '<div style="text-align:center; padding:20px; color:var(--text-sub);">У вас пока нет списков</div>';
        } else {
            html += lists.map(list => {
                ...
                const safeTitle = list.title.replace(/'/g, "\\'").replace(/"/g, '&quot;');
                const safeIcon = (list.icon || '📁').replace(/'/g, "\\'").replace(/"/g, '&quot;');
                ...
                <div class="collection-item-card">
                    <div onclick="window.setCollectionFilter('${list.id}')" style="flex: 1; display: flex; align-items: center; gap: 10px; min-width: 0; cursor: pointer;" title="Открыть список">
                        ...
                        <div style="display: flex; flex-direction: column; min-width: 0;">
                            ...
                            <span style="font-size: 11px; color: var(--text-sub); margin-top: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${list.is_public ? '🌐 Публичный' : '🔒 Личный'} • Слов: ${listItems[list.id]?.size || 0}</span>
                        </div>
                    </div>
  ...
  if (select) {
      client.auth.getSession().then(({ data: { session } }: any) => {
        const myLists = userLists.filter(l => l.user_id === session?.user?.id);
        select.innerHTML = '<option value="" disabled selected>-- Выберите список --</option>' + 
            '<option value="create-new-list" style="font-weight:bold; color:var(--primary);">➕ Создать новый список...</option>' +
            myLists.map(l => `<option value="${l.id}">${l.title}</option>`).join('');
      });
  }
  ...
  if (filterBtn) {
      if (currentCollectionFilter) {
          if (currentCollectionFilter === 'uncategorized') {
              ...
              const list = userLists.find(l => l.id === currentCollectionFilter);
