...
import { parseBilingualString, speak, showToast, debounce } from '../utils/utils.ts';
...
function getFilteredData(): Word[] {
    ...
    return source.filter((w) => {
        if (!w) return false;
        if (state.currentStar !== 'all' && state.currentStar !== 'favorites' && state.currentStar !== 'mistakes' && w.level !== state.currentStar) return false;
        if (state.currentStar === 'favorites' && !state.favorites.has(w.id)) return false;
        if (state.currentStar === 'mistakes' && !state.mistakes.has(w.id)) return false;
        if (w.type !== state.currentType) return false;
        const wTopic = w.topic || w.topic_ru || w.topic_kr;
        const topics = Array.isArray(state.currentTopic) ? state.currentTopic : [state.currentTopic];
        if (!topics.includes('all') && !topics.includes(wTopic)) return false;
        const wCat = w.category || w.category_ru || w.category_kr;
        if (state.currentCategory !== 'all' && wCat !== state.currentCategory) return false;
        return true;
    });
}
...
function initGridVirtualScroll(grid: HTMLElement) {
    ...
    renderVisibleGridItems({ target: grid, sourceData, contentContainer: content, sizer });
}

function renderVisibleGridItems(params: any) {
    const grid = (params.target || params.currentTarget) as HTMLElement;
    if (!grid) return;
    
    const sourceData = params.sourceData || getFilteredData();
    const content = params.contentContainer || document.getElementById('virtual-grid-content');
    ...
    const gap = 15;
    const colCount = Math.max(1, Math.floor((gridWidth + gap) / (MIN_COL_WIDTH + gap)));
    ...
    const endIndex = Math.min(sourceData.length, startIndex + (visibleRows * colCount));
...
function renderVisibleListItems(params: any) {
    const grid = (params.target || params.currentTarget) as HTMLElement;
    if (!grid) return;
...
function createCardFront(item: Word): HTMLElement {
    ...
    speakBtn.onclick = (e) => { 
        ...
        let url = item.audio_url; if (state.currentVoice === 'male' && item.audio_male) url = item.audio_male; 
    ...
    const formatBi = (obj: {kr: string, ru: string}) => (obj.kr && obj.ru && obj.kr !== obj.ru) ? `${obj.kr} (${obj.ru})` : (obj.kr || obj.ru);
...
function createCardBack(item: Word): HTMLElement {
    ...
    if (imgUrl) { img.src = imgUrl; img.onerror = (e) => { const target = (e.target as HTMLElement); if (target) target.style.display = 'none'; }; } else { img.style.display = 'none'; }
    ...
    if (item.isLocal) {
        ...
        delBtn.onclick = (e) => { e.stopPropagation(); import('./ui_custom_words.ts').then(m => m.deleteCustomWord(item.id)); };
    ...
    practiceBtn.onclick = (e) => { e.stopPropagation(); (window as any).checkPronunciation(item.word_kr, practiceBtn); }; 
...
function createListItem(item: Word): HTMLElement {
    ...
    speakBtn.onclick = (e) => { e.stopPropagation(); speak(item.word_kr, item.audio_url || null); };
...
export function markLearned(id: string | number) {
    ...
    state.learned.add(id); state.mistakes.delete(id);
    recordAttempt(id, true); state.dirtyWordIds.add(id);
    addXP(10); checkAchievements(); saveAndRender();
}

export function markMistake(id: string | number) {
    ensureSessionStarted();
    state.mistakes.add(id); state.learned.delete(id);
    recordAttempt(id, false); state.dirtyWordIds.add(id);
    addXP(-5); saveAndRender();
}

export function toggleFavorite(id: string | number, btn?: HTMLElement) {
    if (state.favorites.has(id)) { state.favorites.delete(id); if(btn) { btn.textContent = '🤍'; btn.classList.remove('active'); } }
    else { state.favorites.add(id); if(btn) { btn.textContent = '❤️'; btn.classList.add('active'); } showToast('Добавлено в избранное'); }
...
export function resetProgress(id: string | number) {
    state.learned.delete(id); state.mistakes.delete(id);
    delete state.wordHistory[id]; state.dirtyWordIds.add(id);
