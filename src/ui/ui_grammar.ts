...
export function openGrammarModal(item: Word) {
  ...
  let html = `<div class="grammar-meaning">${escapeHtml(item.translation)}</div>`;

  if (item.grammar_info) {
    // Преобразуем переносы строк в <br> для читаемости
    const formattedInfo = escapeHtml(item.grammar_info).replace(/\n/g, "<br>");
    html += `<div class="grammar-rule-box"><h3>📋 Правила и нюансы</h3><div class="grammar-text">${formattedInfo}</div></div>`;
  }

  if (item.example_kr) {
    html += `<div class="grammar-examples"><h3>💬 Примеры</h3><div class="grammar-ex-item"><div class="ex-kr">${escapeHtml(item.example_kr)}</div><div class="ex-ru">${escapeHtml(item.example_ru || "")}</div></div></div>`;
  }
