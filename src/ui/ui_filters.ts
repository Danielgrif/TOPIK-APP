...
export function populateCategoryFilter() {
    ...
    state.dataStore.forEach((w: Word) => {
        if (w.type !== state.currentType) return;
        const t = w.topic || w.topic_ru || w.topic_kr;
        if (!t || (!state.currentTopic.includes('all') && !state.currentTopic.includes(t))) return;
    ...
    categorySelect.value = 'all'; state.currentCategory = 'all';
}

export function handleTopicChange(val: string) { /* Deprecated for multi-select */ }

export function handleCategoryChange(val: string) { state.currentCategory = val; render(); }
