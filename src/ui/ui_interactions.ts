...
function handleGesture(startX: number, startY: number, endX: number, endY: number, target: EventTarget) {
...
export function saveSearchHistory(query: string) {
    ...
    if (state.searchHistory.length > 5) state.searchHistory = state.searchHistory.slice(0, 5);
    
    localStorage.setItem('search_history_v1', JSON.stringify(state.searchHistory));
}
