...
export function openReviewMode() {
    try {
        ...
        if (!queue || queue.length === 0) { showToast('Нет слов для повторения.'); return; }
        buildReviewModal(queue);
    } catch (e) { console.error(e); showToast('Ошибка при запуске режима повторения'); }
}
...
export function buildReviewModal(queue: Word[]) {
    ...
    if (!modalEl) {
        modalEl = document.createElement('div'); modalEl.id = modalId; modalEl.className = 'modal'; modalEl.setAttribute('role','dialog'); modalEl.setAttribute('aria-modal','true');
    ...
    if (!counter) {
        ...
        if (header) {
            counter = document.createElement('span'); counter.id = 'review-counter'; (counter as HTMLElement).style.cssText = 'font-size:0.6em; opacity:0.7; margin-left:10px;';
    ...
    function renderOne() {
        ...
        const title = document.createElement('div'); title.className = 'quiz-question'; title.textContent = w.word_kr || (w as any).word || '—';
        ...
        const fmt = (d: number) => { if (d === 0) return 'сейчас'; if (d >= 365) return (d/365).toFixed(1) + ' г'; if (d >= 30) return (d/30).toFixed(1) + ' мес'; return d + ' д'; };
    ...
    function renderSummary() {
        ...
        if (stats.remembered > stats.forgotten && typeof (window as any).confetti === 'function') (window as any).confetti({ particleCount: 100, spread: 70, origin: { y: 0.6 }, zIndex: 20005 });
    }
