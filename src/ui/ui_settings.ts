...
export function toggleVoice() { 
    ...
    updateVoiceUI(); 
    showToast(`Голос: ${state.currentVoice === 'female' ? 'Женский' : 'Мужской'}`); 
    scheduleSaveState();
}
...
export function updateVoiceUI() { 
    ...
    if (btn) btn.textContent = state.currentVoice === 'female' ? '👩 Женский' : '👨 Мужской'; 
}
...
export function applyAccentColor() {
    ...
    const color = (THEME_COLORS as any)[colorKey] || THEME_COLORS.purple;
    document.documentElement.style.setProperty('--primary', color);
    // Update active state in UI if selector exists
    document.querySelectorAll('.color-option').forEach(btn => {
        if (btn instanceof HTMLElement) btn.classList.toggle('active', btn.getAttribute('data-color') === state.themeColor);
    });
}
...
export function toggleBackgroundMusic(el?: HTMLInputElement) {
    state.backgroundMusicEnabled = el ? el.checked : !state.backgroundMusicEnabled;
...
export function populateMusicTrackSelect() {
    ...
    state.MUSIC_TRACKS.forEach((track: any) => {
        const option = document.createElement('option');
        option.value = track.filename;
        option.textContent = track.name;
        // @ts-ignore
        if (track.filename === (state as any).backgroundMusicTrack) {
...
export function applyBackgroundMusic(forcePlay: boolean = false) {
    const playerA = document.getElementById('music-player-a') as HTMLAudioElement | null;
    const playerB = document.getElementById('music-player-b') as HTMLAudioElement | null;
    ...
    if (!hasInteracted && state.backgroundMusicEnabled && forcePlay) {
        ...
        if (activePlayer.volume < state.backgroundMusicVolume) {
            crossfade(activePlayer, null, state.backgroundMusicVolume, activePlayer.volume);
        }
    }
    ...
    const targetTrack = state.MUSIC_TRACKS.find((t: any) => t.id === trackId);
 ...
 */
function crossfade(fadeInPlayer: HTMLAudioElement, fadeOutPlayer: HTMLAudioElement | null, finalVolume: number, startVolume?: number) {
...
export function duckBackgroundMusic(duck: boolean) {
    const activePlayer = document.getElementById(activePlayerId === 'a' ? 'music-player-a' : 'music-player-b') as HTMLAudioElement | null;
    if (!activePlayer || !state.backgroundMusicEnabled || activePlayer.paused) return;

    if (volumeAnimationInterval) {
        clearInterval(volumeAnimationInterval);
    }

    const targetVolume = duck ? state.backgroundMusicVolume * 0.2 : state.backgroundMusicVolume;
