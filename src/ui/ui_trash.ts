...
let selectedTrashIds = new Set<number>();
...
function renderTrashList() {
    ...
    if (deletedWords.length === 0) {
        ...
        container.innerHTML = deletedWords.map(word => {
...
export function setupTrash() {
    ...
    if (trashModal) {
        ...
        observer.observe(trashModal, { attributes: true, attributeFilter: ['class'] });
    }
    ...
    const selectAllCheckbox = document.getElementById('trash-select-all-checkbox') as HTMLInputElement;
...
function updateTrashActionButtons() {
    const emptyBtn = document.getElementById('empty-trash-btn') as HTMLButtonElement;
    const restoreAllBtn = document.getElementById('restore-all-btn') as HTMLButtonElement;
    const selectAllContainer = document.getElementById('trash-select-all-container');
    const selectAllCheckbox = document.getElementById('trash-select-all-checkbox') as HTMLInputElement;

    if (!emptyBtn || !restoreAllBtn || !selectAllContainer || !selectAllCheckbox) return;
    ...
    if (selectAllCheckbox) {
        selectAllCheckbox.checked = hasDeletedWords && selectedCount === deletedWords.length;
        selectAllCheckbox.indeterminate = selectedCount > 0 && selectedCount < deletedWords.length;
    }
}
...
async function bulkPermanentDelete() {
    const idsToDelete = selectedTrashIds.size > 0 ? Array.from(selectedTrashIds) : deletedWords.map(w => w.id as number);
    ...
    openConfirm(
        ...
        async () => {
            ...
            deletedWords = deletedWords.filter(w => !idsToDelete.includes(w.id as number));
            selectedTrashIds.clear();
            renderTrashList();
...
async function bulkRestore() {
    const idsToRestore = selectedTrashIds.size > 0 ? Array.from(selectedTrashIds) : deletedWords.map(w => w.id as number);
    ...
    openConfirm(`Восстановить ${idsToRestore.length} слов(а)?`, async () => {
        ...
        const wordsToRestore = deletedWords.filter(w => idsToRestore.includes(w.id as number));

        // Optimistic UI update
        deletedWords = deletedWords.filter(w => !idsToRestore.includes(w.id as number));
        ...
        // Server update
        const { error } = await client.from('vocabulary').update({ deleted_at: null }).in('id', idsToRestore);
        if (error) {
            showToast(`❌ Ошибка восстановления: ${error.message}`);
            // Revert UI on error
            deletedWords.push(...wordsToRestore);
            state.dataStore = state.dataStore.filter(w => !idsToRestore.includes(w.id as number));
            renderTrashList();
            render();
        }
    });
