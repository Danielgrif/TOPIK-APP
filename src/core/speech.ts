...
export function checkPronunciation(correctWord: string, btn?: HTMLElement, onResult?: (similarity: number, text: string) => void) {
    ...
    rec.onend = () => {
        if (btn) { btn.textContent = '🗣️'; (btn as HTMLButtonElement).disabled = false; }
    };
