import { WordHistoryItem, SM2State } from '../types/index.ts';
...
export const Scheduler = {
    ...
    init({ dataStore, wordHistory }: { dataStore: any[], wordHistory: Record<string | number, WordHistoryItem> }) {
        this._data = dataStore || [];
        this._history = wordHistory || {};
    },

    calculate(grade: number, item: SM2State | null | undefined, lastReviewTime: number | null = null): ReviewResult {
    ...
    previewNextIntervals(wordKey: string | number): { fail: number, hard: number, easy: number } {
