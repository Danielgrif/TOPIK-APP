...
import { duckBackgroundMusic } from "./src/ui/ui_settings";
...
export async function runSettingsTests(
  ...
  // 0.5 * 0.2 = 0.1
  assert(
    "duckBackgroundMusic(true) lowers volume",
    Math.abs(mockPlayer.volume - 0.1) < 0.001,
  );

  // Test: Restore volume
  duckBackgroundMusic(false);
  assert(
    "duckBackgroundMusic(false) restores volume",
    Math.abs(mockPlayer.volume - 0.5) < 0.001,
  );
